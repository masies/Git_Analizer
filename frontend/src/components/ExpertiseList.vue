		...
		<div class="row list-group">
			<div class="col-12 list-group-item list-group-item-header">
				<div class="row font-weight-bold">
					<div class="col-9">
						<span @click="sortBy('developerMail')">Developer</span>
					</div>
					<div class="col-3">
						<span @click="sortBy('expertise')">Expertise</span>
					</div>
				</div>
			</div>
	...
	export default {
		...
		methods: {
			loadData: function() {
				...
				this.data.content = [
				{
					developerMail: "myUser",
					expertise: 24,
				},
				{
					developerMail: "mySecondUser",
					expertise: 33,
				}]
				// fetch(`/api/repo/${this.$route.params.owner}/${this.$route.params.name}/expertise${this.queryString}&page=${this.currentPage-1}&size=${this.size}`)
				// .then(response => {
