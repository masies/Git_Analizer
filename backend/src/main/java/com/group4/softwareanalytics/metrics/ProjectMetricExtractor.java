...
public class ProjectMetricExtractor {
    ...
    public static ProjectMetric commitCodeQualityExtractor(String owner, String repoName, String commit, List<String> parentCommits){
        String path = "./repo/" + owner + File.separator + repoName;

        try (Git git = Git.open(new File(path + "/.git"))) {
            git.checkout().setName(commit).call();
            List<Float> metrics = classMetricsExtractor(path);
            if (parentCommits.size() == 1) {
                git.checkout().setName(parentCommits.get(0)).call();
                List<Float> parentMetrics = classMetricsExtractor(path);
